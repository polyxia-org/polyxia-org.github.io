...
For the recognition of this wake word, we use the [Mycroft Precise]() Python library that is a lightweight, simple-to-use,
...
To do so, we rely on OpenAI's whisper model. is an automatic speech recognition (ASR) system trained on 680,000 hours of supervised multilingual
and multitasking data collected from the web. More precisely we use the openai/whisper-base model which has been trained with 74 million parameters.
